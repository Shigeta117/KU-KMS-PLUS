  ...
  return (
    <div className="min-h-dvh flex flex-col w-full sm:max-w-3xl sm:mx-auto bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-100">
      ...
      <header className="px-6 py-4 flex items-center justify-between border-b border-slate-100 dark:border-slate-800">
        ...
        <Link
          href="/login"
          className="text-sm font-semibold text-ku-accent dark:text-blue-400 hover:opacity-80 transition-opacity"
        >
          ログイン
        </Link>
      </header>
      ...
      <main className="flex-1">
        ...
        <section className="px-6 py-16 md:py-24 text-center space-y-6">
          ...
          <div className="pt-4 flex flex-col sm:flex-row items-center justify-center gap-3">
            <Link
              ...
              className="w-full sm:w-auto px-6 py-3 rounded-xl font-bold text-white shadow-lg hover:shadow-xl transition-all flex items-center justify-center gap-2 bg-ku-blue hover:bg-ku-blue-dark"
            >
              <Download size={18} />
              Chrome に追加（無料）
            </Link>
          ...
          <p className="text-xs text-slate-400 dark:text-slate-500 pt-2">
            <a href="#setup" className="underline underline-offset-2 hover:text-slate-600 dark:hover:text-slate-300 transition-colors">
              導入手順を確認する
            </a>
          </p>
        </section>
        ...
        <section id="setup" className="px-6 py-16 max-w-2xl mx-auto space-y-8">
          ...
          <div className="space-y-6">
            <StepCard number="1" title="Chrome 拡張機能をインストール">
              <p className="text-sm text-slate-600 dark:text-slate-400 mb-4">
                Chrome ウェブストアからワンクリックでインストールできます。
              </p>
              <Link
                href={CHROME_STORE_URL}
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-bold text-white bg-ku-blue hover:bg-ku-blue-dark transition-colors"
              >
            ...
            <StepCard number="2" title="アカウントの設定">
              ...
              <Link
                href="/signup"
                className="text-sm font-bold text-ku-accent dark:text-blue-400 flex items-center gap-1 hover:underline"
              >
...
function StepCard({ number, title, children }: { number: string; title: string; children: ReactNode }) {
  return (
    <div className="flex gap-4">
      <div className="flex-shrink-0 w-8 h-8 rounded-full bg-ku-blue text-white flex items-center justify-center font-bold text-sm">
        {number}
      </div>
      <div className="pt-1">
