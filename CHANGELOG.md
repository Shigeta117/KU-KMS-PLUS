...
## [1.4.1] — 2026-05-26（カルーセルの自動ループ化）

### Changed
- ランディングページのプロモーション画像を、10秒ごとの自動スライド＆無限ループに対応したカルーセルに変更

---

## [1.4.0] — 2026-05-26（ランディングページ改善・オフラインページ修正）

### Added
- ランディングページにプロモーション画像ギャラリーを追加

### Fixed
- オフラインページの古いアイコン（K+）を新しいデザインのアイコンに修正

---

## [1.3.1] — 2026-05-25（PC / タブレットレスポンシブ刷新）

### Changed
- ダッシュボード（`/app`）を PC・タブレット（768px 以上）で 2 カラムレイアウトに変更
  - FilterBar がサイドバーに変化（左 256px 固定、sticky、縦並びタブリスト）
  - メインコンテンツが右側に拡張（max-width 1024px）
  - カテゴリチップが PC サイドバーでは折り返し表示に変更
- 設定ページの最大幅を PC で `max-w-4xl` に拡張
- 設定ページのセクションを PC で 2 カラムグリッドレイアウトに変更（ブックマークレットセクションは全幅）

---

## [1.3.0] — 2026-05-25（メモ機能追加・「スキップ」ラベル変更）

### Added
- 課題ごとにメモを追加・編集できる機能を実装
  - Web ダッシュボード（TaskCard）: タップでインライン編集、フォーカスアウトで自動保存
  - LMS 拡張機能（content.js）: 各課題ブロックのアクションバー下部にメモ入力欄を追加
  - DB: `assignments` テーブルに `note TEXT DEFAULT NULL` カラムを追加
  - スクレイプ時に既存メモは上書きされない（`is_completed_manual` / `is_hidden` と同方式）

### Changed
- 「非表示」ラベルを「スキップ」に統一（Web UI・LMS 拡張機能ともに変更）
  - FilterBar タブ: `非表示` → `スキップ`
  - TaskCard アクションボタン: `非表示` → `スキップ`
  - EmptyState メッセージ: `非表示にした課題はありません` → `スキップした課題はありません`
  - content.js ボタン: `非表示` → `スキップ`
  - トースト通知: `非表示にしました` → `スキップしました`
